// MV3 service worker: schedules agent runs.
// Prefers an offscreen document to call Prompt API; falls back to SW if needed.
...
async function runAgentOnceInternal(reasonLabel = "") {
    ...
    // Prefer offscreen (more stable Prompt API context)
    ...
    // Notify
    let iconUrl = chrome.runtime.getURL("assets/icon-128.png");
    try {
        await chrome.notifications.create({
            type: "basic",
            iconUrl,
            title: "Spark drafts ready",
            message: `3 new ${(cfg.platform || "post")} drafts generated for: ${topic}` + (usedOffscreen ? "" : " (SW fallback)") + (reasonLabel ? ` — ${reasonLabel}` : ""),
            priority: 1
        });
    } catch (e) {
